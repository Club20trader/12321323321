...
// VIP access
// =============================
// VIP is enabled for every visitor of this frontend.
// Keep the flag in one place so all UI checks can use the same value.
const HAS_VIP_ACCESS = true;
try { localStorage.setItem("vipAccess", "true"); } catch (_) {}
...
(function(){
    const MODELS = [
        ...
        { id: "NE_V2", label: "NeuralEdge V2", disabled: false, note: "VIP" }
    ];
    ...
    `).join("");
        ...
        grid.querySelectorAll(".md-chip").forEach(btn=>{
            btn.addEventListener("click", ()=>{
                ...
                // VIP is enabled for everyone, so V2 can be selected directly.
                selectedId = item.label;
                setModelUI(item);
                close();
            });
